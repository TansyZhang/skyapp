...
- (void) viewDefaultSetting{
    ...
    //find minScale by min. size = 10
    //double minScaleW = SCALE_MIN_PIC / self.bounds.size.width;
    //double minScaleH = SCALE_MIN_PIC / self.bounds.size.height;
    //minScale = (minScaleW > minScaleH) ? minScaleW : minScaleH;
    
    //double maxScaleW = SCALE_MAX_PIC / self.bounds.size.width;
    //double maxScaleH = SCALE_MAX_PIC / self.bounds.size.height;
...
{
    BOOL result = NO;
    if(@selector(delete) == action || @selector(openLink) == action || @selector(toLayerTop) == action || @selector(toLayerUp) == action || @selector(toLayerDown) == action || @selector(toLayerBottom) == action) {
        result = YES;
    }
    return result;
}
